...
- (void)savedPhotoImage:(UIImage*)image didFinishSavingWithError: (NSError *)error contextInfo: (void *)contextInfo {
    if (error) {
        [self makeToast:LocalizationKey(@"保存图片失败") duration:1.5 position:CSToastPositionCenter];
    }
    else {
//        NSLog(NSLocalizedString(@"保存图片成功", nil));
        [self makeToast:LocalizationKey(@"保存图片成功") duration:1.5 position:CSToastPositionCenter];
    }
}
