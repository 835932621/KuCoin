...
//    [self RightsetupNavgationItemWithpictureName:@"zicanliushui"]; 屏蔽掉右上角的筛选按钮
...
//    [self.uitableView registerNib:[UINib nibWithNibName:@"CurrencyrecordTableViewCell" bundle:nil] forCellReuseIdentifier:@"CurrencyrecordTableViewCell"];
    [self.uitableView registerNib:[UINib nibWithNibName:@"ChargerecordTableViewCell" bundle:nil] forCellReuseIdentifier:@"ChargerecordTableViewCell"];
    self.pageNo = 0;
...
-(void)getadata{
    ...
    [MineNetManager reqWithdrawRecord:^(id resPonseObj, int code) {
        [EasyShowLodingView hidenLoding];
        ...
        if (code) {
            if (code == 1 && [resPonseObj[@"code"] isEqual:@0]) {
                ...
                NSArray *dataArr = [RecordModel mj_objectArrayWithKeyValuesArray:resPonseObj[@"data"]];
