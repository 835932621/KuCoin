...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = LocalizationKey(@"个人中心");//[[ChangeLanguage bundle] localizedStringForKey:@"securitySetting" value:nil table:@"English"];
...
-(NSArray *)getNameArr{
    NSArray * nameArr = @[@"",[[ChangeLanguage bundle] localizedStringForKey:@"identityAuthentication" value:nil table:@"English"],[[ChangeLanguage bundle] localizedStringForKey:@"bindPhoneNumber" value:nil table:@"English"],[[ChangeLanguage bundle] localizedStringForKey:@"loginPassword" value:nil table:@"English"],[[ChangeLanguage bundle] localizedStringForKey:@"moneyPassword" value:nil table:@"English"],LocalizationKey(@"获取API"),];
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            AccountSettingTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([AccountSettingTableViewCell class])];
                ...
                if (indexPath.row == 1) {
                    if (_realVerified) {
                        cell.rightLabel.text = LocalizationKey(@"verification");
                        cell.rightLabel.textColor = baseColor;
                    }else{
                        if ([_accountInfo.realAuditing isEqualToString:@"1"]) {
                            //审核中
                            cell.rightLabel.text = LocalizationKey(@"auditing");
                            cell.rightLabel.textColor = RGBOF(0x999999);
                        }else{
                            if (_accountInfo.realNameRejectReason== nil) {
                                //未绑定
                                cell.rightLabel.text = LocalizationKey(@"unverified");
                                cell.rightLabel.textColor = RGBOF(0x999999);
                            }else{
                                cell.rightLabel.text = LocalizationKey(@"auditFailure");
                                cell.rightLabel.textColor = RGBOF(0x999999);
                            }
                        }
                    }
...
//        identityVC.identifyStatus = @"0";
        ...
        GetApiViewController *getApiVC = [[GetApiViewController alloc] init];
        getApiVC.hidesBottomBarWhenPushed = YES;
        ...
        [self.navigationController pushViewController:getApiVC animated:YES];
    }else if (indexPath.row == 3){
        //登录密码
        if (!_phoneVerified) {
...
-(void)headImage:(NSString *)urlString{
    ...
    [MineNetManager setHeadImageForUrl:urlString CompleteHandle:^(id resPonseObj, int code) {
        ...
        if (code) {
            if ([resPonseObj[@"code"] integerValue] == 0) {
                //设置头像成功
                 [self.view makeToast:LocalizationKey(@"设置头像成功") duration:1.5 position:CSToastPositionCenter];
