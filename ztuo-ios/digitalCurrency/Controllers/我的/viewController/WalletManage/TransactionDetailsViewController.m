...
- (void)viewDidLoad {
    ...
    self.title = LocalizationKey(@"交易详情");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row % 2) {
        ...
        cell.backView.backgroundColor = kRGBColor(244, 245, 246);
    }
    return cell;
}
...
- (NSMutableArray *)dataSourceArr{
    if (!_dataSourceArr) {
        ...
        for (int i = 0; i < 30; i++) {
            NSDictionary *dic = @{@"tradeTime":@"2018-07-03 10:46:33", @"tradePair":@"BHB/USTD", @"tradeDirection":LocalizationKey(@"限价卖出"), @"tradePrice":@"0.00001USTD", @"tradeOrderVolume":@"1000BHB", @"tradeDeal":@"1000BHB", @"tradeServiceCharge":@"0.001USTD"};
...
- (UIScrollView *)scrollView{
    if (!_scrollView) {
        ...
        _scrollView.layer.borderColor = kRGBColor(233, 234, 235).CGColor;
...
- (UIView *)sectionView{
    if (!_sectionView) {
        ...
        for (int i = 0; i < titles.count; i++) {
            ...
            label.textColor = kRGBColor(118, 119, 120);
            if (i == 0) {
