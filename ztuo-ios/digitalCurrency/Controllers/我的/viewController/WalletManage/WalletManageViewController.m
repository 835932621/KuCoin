...
@property(nonatomic, strong)BeforeChargeModel *beforeTransferModel;
...
-(void)getData{
    ...
    [MineNetManager getMyWalletInfoForCompleteHandle:^(id resPonseObj, int code) {
        ...
        NSLog(LocalizationKey(@"请求币币账户资产的接口 --- %@"),resPonseObj);
//        [EasyShowLodingView hidenLoding];
        if (code) {
            if ([resPonseObj[@"code"] integerValue] == 0) {
                ...
                for (int i=0; i<self.walletManageArr.count; i++) {
                    WalletManageModel *model = self.walletManageArr[i];
                    if ([model.coin.name isEqualToString:@"USDT"]) {//usdt排序放到首位
                        [self.walletManageArr exchangeObjectAtIndex:i withObjectAtIndex:0];
                        break;
                    }
                }
...
-(void)updateTotalAsset{
    ...
    for (WalletManageModel *walletModel in self.walletManageArr) {
        //计算总资产
        ass1 += [walletModel.balance floatValue]*[walletModel.coin.usdRate floatValue];
        ass1 += [walletModel.frozenBalance floatValue]*[walletModel.coin.usdRate floatValue];
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if ([tableView isEqual:_heYueTableView]) {
        if(self.curSelectedAccountBtn.tag == 105){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:_heYueTableView]) {
        ...
        if(self.curSelectedAccountBtn.tag == 105){
            ...
            [cell setMiaoHeYueData:self.mhyDataDic];
        }
...
- (void)loadWalletMenuView {
    ...
    NSArray *menutitles =@[LocalizationKey(@"chargeMoney"),LocalizationKey(@"mentionMoney")];//@[LocalizationKey(@"chargeMoney"),LocalizationKey(@"mentionMoney"),LocalizationKey(@"转账"),LocalizationKey(@"tv_overturn")];
    NSArray *choiceTitles=@[LocalizationKey(@"tv_coins")];//@[LocalizationKey(@"tv_coins"),LocalizationKey(@"tv_constract"),LocalizationKey(@"秒合约账户")];
    ...
    CGSize cbtnsize=CGSizeMake((SCREEN_WIDTH_S-30-interval*(2))/3,30);
    ...
    _heYueTableView=[[UITableView alloc]init];
    [_heYueTableView  setSeparatorStyle:UITableViewCellSeparatorStyleNone];
    _heYueTableView.backgroundColor = mainColor;
    _heYueTableView.delegate=self;
    _heYueTableView.dataSource=self;
    _heYueTableView.showsVerticalScrollIndicator=NO;
    _heYueTableView.alwaysBounceVertical=YES;
    if (@available(iOS 11.0, *)) {
        
        _heYueTableView.estimatedRowHeight = 0;
        
        _heYueTableView.estimatedSectionFooterHeight = 0;
        
        _heYueTableView.estimatedSectionHeaderHeight=0;
        _heYueTableView.contentInsetAdjustmentBehavior= UIScrollViewContentInsetAdjustmentNever;
    }
    
    [self.view addSubview:_heYueTableView];
    [_heYueTableView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.mas_equalTo(0);
        make.top.equalTo(menubackview.mas_bottom).offset(0);
        make.bottom.mas_equalTo(-SafeAreaBottomHeight);
    }];
    
    UIButton *button = [_menuView viewWithTag:102];
    self.curSelectedAccountBtn = button;
...
{
//    NSLog(@"darker index:  %ld", buttonIndex);
    if(buttonIndex == 1){
        CGFloat count = [[self.alertTextField text] floatValue];
        if(count <= 0){
            return;
        }
        
        RechargeAssetsViewController *vc = [RechargeAssetsViewController new];
        vc.beforeChargeModel = self.beforeTransferModel;
        vc.chargeCount = self.alertTextField.text;
        [self.navigationController pushViewController:vc animated:YES];
        
        /*[MineNetManager reqRecharge:count CompleteHandle:^(id resPonseObj, int code) {
            
            NSLog(LocalizationKey(@"darker 请求充值返回 %@"),resPonseObj);
            if (code) {
                if (code == 1 && [resPonseObj[@"code"] isEqual:@0]) {
//                    NSURL *URL = [NSURL URLWithString:resPonseObj[@"data"]];
...
- (BOOL)alertViewShouldEnableFirstOtherButton:(UIAlertView *)alertView {
    ...
    return [self.alertTextField.text floatValue]>=self.beforeTransferModel.minTransferValue;
...
-(void)menubtnclick:(UIButton *)button {
    ...
    switch (button.tag) {
        case 100:{ //充币
            [MineNetManager reqBeforeCharge:^(id resPonseObj, int code) {
                
                if (code) {
                    if ([resPonseObj[@"code"] integerValue] == 0) {
                        NSDictionary *dic = resPonseObj[@"data"];
                        BeforeChargeModel *model = [BeforeChargeModel mj_objectWithKeyValues:dic];
                        self.beforeTransferModel = model;
                        if(model.transferStatus == 1){
                            
                            NSString *hint = [NSString stringWithFormat:@"Minimum deposit amount %.2f（Include a decimal for checking the order amount）", model.minTransferValue];
                            
                            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"Please enter the amount(USDT)" message:hint delegate:self cancelButtonTitle:LocalizationKey(@"取消") otherButtonTitles:LocalizationKey(@"确定"),nil];
                            
                            alert.alertViewStyle = UIAlertViewStylePlainTextInput;// 设置输入框的键盘类型
                            self.alertTextField = [alert textFieldAtIndex:0];
                            self.alertTextField.placeholder = LocalizationKey(@"数量");
                //            self.alertTextField.text = @"100";
                            self.alertTextField.keyboardType = UIKeyboardTypeDecimalPad;
                            [alert show];
                        }
                        else{
                            RechargeAssetsViewController *vc = [RechargeAssetsViewController new];
...
//          [self.view makeToast:LocalizationKey(@"请联系管理人员") duration:1.5 position:CSToastPositionCenter];
...
//            _refreshFlag=YES;
//
//            TransferAssetsViewController *vc = [TransferAssetsViewController new];
//            CGFloat usdtCount = 0.0;
//            for (WalletManageModel *walletModel in self.walletManageArr) {
//                //计算usdt可用资产
//                usdtCount += [walletModel.balance floatValue]*[walletModel.coin.usdRate floatValue];
//            }
//            vc.usdtCount = usdtCount;
//            [self.navigationController pushViewController:vc animated:YES];
//
//        }
//            break;
...
//            _refreshFlag=YES;
//
//            AssetTransferViewController *vc=[[AssetTransferViewController alloc]init];
////            vc.contractAccountArray=self.contractDataArray;
//            [self.navigationController pushViewController:vc animated:YES];
//
//        }
//            break;
        case 102:{ //币币账户
            ...
            self.curSelectedAccountBtn.selected=NO;
            self.backView.hidden=NO;
            self.tableview.hidden=NO;
            self.heYueTableView.hidden=YES;
            button.selected=YES;
            self.curSelectedAccountBtn = button;
...
//            }];
//
//            [self.heYueTableView reloadData];
//        }
//            break;
//        case 106:{ //秒合约账户
//            self.curSelectedAccountBtn.selected=NO;
