...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.dataArray.count!=0&&indexPath.row<self.dataArray.count) {
        if (_type==2) {
            TransferCurrencyModel *model = self.dataArray[indexPath.row];
            
            NSInteger type = model.type;
            NSString *symbol = model.symbol;
            if(type == 0){//币币账户
                cell.mtitlelabel.text = LocalizationKey(@"币币账户");
            }
            else if(type == 1){//永续账户
                cell.mtitlelabel.text = [NSString stringWithFormat:LocalizationKey(@"%@ 永续账户"), symbol];
            }
            else{
                cell.mtitlelabel.text = LocalizationKey(@"秒合约账户");
            }
