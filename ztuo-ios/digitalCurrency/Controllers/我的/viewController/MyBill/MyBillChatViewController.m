...
- (IBAction)sendBtnClick:(UIButton *)sender {
    if ([self.sendTextView.text isEqualToString:@""]) {
       // [self.view makeToast:LocalizationKey(@"不得发送空的内容") duration:1.5 position:CSToastPositionCenter];
        return;
    }
    //发送消息内容
    ...
    NSDictionary *dic;
    if (self.clickIndex == 1) {
        dic=[NSDictionary dictionaryWithObjectsAndKeys:self.groupModel.orderId,@"orderId",self.groupModel.uidTo,@"uidFrom",self.groupModel.uidFrom,@"uidTo",self.groupModel.nameTo,@"nameTo",[YLUserInfo shareUserInfo].username,@"nameFrom",[NSNumber numberWithInt:1],@"messageType",self.sendTextView.text,@"content",[YLUserInfo shareUserInfo].avatar,@"avatar"];
        ...
        dic=[NSDictionary dictionaryWithObjectsAndKeys:self.model.orderSn,@"orderId",self.model.myId,@"uidFrom",self.model.hisId,@"uidTo",self.model.otherSide,@"nameTo",[YLUserInfo shareUserInfo].username,@"nameFrom",[NSNumber numberWithInt:1],@"messageType",self.sendTextView.text,@"content",[YLUserInfo shareUserInfo].avatar,@"avatar"];
        ChatGroupInfoModel *info = [[ChatGroupInfoModel alloc] init];
        info.content = model.content;
...
- (void)ChatdelegateSocket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag{
    ...
    if (cmd==SEND_CHAT) {//发送消息
        if (endStr) {
//             NSLog(LocalizationKey(@"发送消息--%@-收到的回复命令--%d"),endStr,cmd);
        }
    }
...
//            NSLog(LocalizationKey(@"接收消息--收到的回复-%@--%d--"),dic,cmd);
        }

    }else{
//        NSLog(LocalizationKey(@"单聊消息-%@--%d"),endStr,cmd);
    }
}
