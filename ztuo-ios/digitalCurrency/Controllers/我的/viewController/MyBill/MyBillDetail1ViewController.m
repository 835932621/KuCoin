...
-(NSString *)dateTimeDifferenceWithStartTime:(NSString *)startTime endTime:(NSString *)endTime{
    ...
    [date setTimeZone:ChangeLanguage.timeZone];
...
-(void)payCompletedSendMessage{
    NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:self.detailModel.orderSn,@"orderId",self.detailModel.hisId,@"uidFrom",self.detailModel.myId,@"uidTo",self.detailModel.otherSide,@"nameTo",[YLUserInfo shareUserInfo].username,@"nameFrom",[NSNumber numberWithInt:0],@"messageType",@"",@"content",[YLUserInfo shareUserInfo].avatar,@"avatar"];
...
//        [textField setValue:RGBOF(0x999999) forKeyPath:@"_placeholderLabel.textColor"];
    ...
    .LeeAddAction(^(LEEAction *action) {
        ...
        action.title = LocalizationKey(@"确定");
        action.clickBlock = ^{
            //响应事件
            //得到文本信息
    ...
    .LeeAddAction(^(LEEAction *action) {
        action.titleColor = RGBOF(0xe6e6e6);
        action.borderColor = kRGBColor(18, 22, 28);
        action.title = LocalizationKey(@"取消");
    })
    .LeeShow();
...
-(void)complaintsTextField{
    ...
    if (([currentTime integerValue] - [orderTime integerValue]) < 1800000) {
        ...
        .LeeAddContent(^(UILabel *label) {
            label.textColor = RGBOF(0xe6e6e6);
            label.text = LocalizationKey(@"付款后半个小时，才可以申诉");
        })
        .LeeAddAction(^(LEEAction *action) {
            action.titleColor = RGBOF(0xe6e6e6);
            action.borderColor = mainColor;
