...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = LocalizationKey(@"切换账号");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        cell.accountNun.text = [NSString stringWithFormat:LocalizationKey(@"账号：%@"),self.nameArr[indexPath.row]];
        //hookImage
        ...
        cell.textLabel.text = LocalizationKey(@"添加新账号登录");
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
-(void)login:(NSString *)userName withPass:(NSString *)password withIndex:(NSInteger)index{
    [EasyShowLodingView showLodingText:LocalizationKey(@"登录中...")];
