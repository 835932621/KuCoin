...
@property (weak, nonatomic) IBOutlet CustomButton *sendVerifyCodeBtn;
@end

@implementation LoginViewController

///发送验证码
- (IBAction)onSendVerifyCode:(UIButton *)sender {
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",HOST, @"uc/mobile/login/code"];
    ...
    [[XBRequest sharedInstance] postDataWithUrl:url Parameter:param ResponseObject:^(NSDictionary *responseResult) {
        NSError * err = [responseResult objectForKey:@"resError"];
        if (!err) {
            if ([responseResult[@"code"] integerValue] == 0) {
                ...
                dispatch_source_set_event_handler(_timer, ^{
                    if(timeout<=0){ //倒计时结束，关闭
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [self.sendVerifyCodeBtn setTitle:LocalizationKey(@"sendValidate") forState:UIControlStateNormal];
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [self.sendVerifyCodeBtn setTitle: [NSString stringWithFormat:@"%@s",strTime] forState:UIControlStateNormal];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                NSString * str = [[NSString alloc] initWithFormat:@"error %ld",err.code];
                [APPLICATION.window makeToast:str duration:1.5 position:CSToastPositionCenter];
            });
        }
    }];
}
...
//    [_passwordTF setValue:[UIFont boldSystemFontOfSize:12] forKeyPath:@"_placeholderLabel.font"];
    ...
    if(!ISEMPTY_S(UserNameAndPassword)){//输入上次保存的密码
        NSArray *array = [UserNameAndPassword componentsSeparatedByString:@"|"];
        if(array.count == 2){
//            NSString *userName = [YLUserInfo getLoginAccount];
//            if (!ISEMPTY_S(userName)) {
...
- (BOOL)captchaButtonShouldBeginTapAction:(CustomButton *)button {
    ...
    if ([NSString stringIsNull:self.verifyCodeTF.text]) {
        [self.view makeToast:NSLocalizedString(@"请输入验证码", nil) duration:1.5 position:CSToastPositionCenter];
        return NO;
    }
...
//    }
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:secondaryRequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        //NSLog(NSLocalizedString(@"请求头中%@", nil),httpResponse.allHeaderFields);
    ...
    NSDictionary *param = @{@"username":self.userNameTF.text,
                            ...
                            @"verifyCode":self.verifyCodeTF.text
    };
    
    [YLUserInfo saveLoginAccount:self.userNameTF.text];
    
    [[XBRequest sharedInstance] postDataWithUrl:url Parameter:param ResponseObject:^(NSDictionary *responseResult) {
        [EasyShowLodingView hidenLoding];
        NSLog(NSLocalizedString(@"登陆返回 --- %@", nil),responseResult);
        NSError * err = [responseResult objectForKey:@"resError"];
        if (!err) {
...
//                [YLUserInfo saveLoginAccount:[YLUserInfo shareUserInfo].username];
