...
- (void)viewDidLoad {
    ...
    self.agressButton.selected = YES;
//    [self getUserprotocol];
    self.contryCode = LocalizationKey(@"中国");
    
//    [self rightBarItemWithTitle:LocalizationKey(@"emailRegister")]; 屏蔽邮箱注册
...
- (IBAction)onRegisterClick:(UIButton *)sender {
    [self goRegister];
}
...
- (void)delegateGtCaptcha:(GT3CaptchaManager *)manager didReceiveCaptchaCode:(NSString *)code result:(NSDictionary *)result message:(NSString *)message{
    ...
    if (_isEmail) {
        [postResult appendFormat:@"%@", [NSString stringWithFormat:@"email=%@&country=%@",self.phoneNumberTF.text,LocalizationKey(@"中国")]];
    }else {
...
//        }
    ...
    if (_isEmail) {
        [LoginNetManager getAccountForEmail:self.phoneNumberTF.text withpassword:self.passwordTF.text   withcode:self.verifyCodeTF.text withusername:self.userNameTF.text withcountry:LocalizationKey(@"中国") withpromotion:@"" CompleteHandle:^(id resPonseObj, int code) {
            [EasyShowLodingView hidenLoding];
            if (code) {
                if ([resPonseObj[@"code"] integerValue] == 0) {
        ...
        [YLUserInfo saveLoginAccount:self.phoneNumberTF.text];
        [LoginNetManager getAccountForPhone:self.phoneNumberTF.text withpassword:self.passwordTF.text withcode:self.verifyCodeTF.text withusername:self.userNameTF.text withcountry:self.contryCode withG3Result:codeDic CompleteHandle:^(id resPonseObj, int code) {
            ...
            if (code) {
                if ([resPonseObj[@"code"] integerValue] == 0) {
//                    NSLog(LocalizationKey(@"注册成功返回：%@"),resPonseObj[@"data"]);
...
//                    [YLUserInfo getuserInfoWithDic:resPonseObj[@"data"]];//存储登录信息
//                    [[NSUserDefaults standardUserDefaults] setObject:self.phoneNumberTF.text forKey:@"userNameTF"];
//                    [[NSUserDefaults standardUserDefaults] synchronize];
                    
                    [self.view makeToast:LocalizationKey(@"注册成功") duration:1.0 position:CSToastPositionCenter];
