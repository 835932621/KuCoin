...
@interface ContractExchangeViewController ()<StepSliderDelegate,UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,SocketDelegate>{
...
@property (nonatomic, strong) UITextField *inputPriceTextField2;
...
-(UILabel *)topTitlelabel{
    ...
    if (!_topTitlelabel) {
        ...
        _topTitlelabel.frame=CGRectMake(40,3,SCREEN_WIDTH_S-105,30);
...
- (void)viewDidLoad {
    ...
    //self.topTitlelabel.text=NSLocalizedString(@"BTC/USDT 永续", nil);
    ...
    self.myscollView.contentSize=CGSizeMake(SCREEN_WIDTH_S,self.bottomBackView.origin.y+self.bottomBackView.frame.size.height + NEW_StatusBarHeight+MULTI_CONTRACT_TABBAR_HEIGHT);
    _isinitFrist=YES;
...
#pragma mask - 初始化顶部的tabbar
-(void) initTopView{
    
    [self.view addSubview:self.selectCurencyBtn];
    
    UIView *line=[[UIView alloc]initWithFrame:CGRectMake(0,self.selectCurencyBtn.frame.origin.y+self.selectCurencyBtn.frame.size.height+8,SCREEN_WIDTH_S, 1)];
    line.backgroundColor=ViewBackgroundColor;
...
-(void)initleftView{
    ...
    _inputPriceTextField2=[[UITextField alloc]init];
    _inputPriceTextField2.text = NSLocalizedString(@"以最优价格买入", nil);
    _inputPriceTextField2.textColor=AppTextColor_Level_2;
    _inputPriceTextField2.font=[UIFont systemFontOfSize:12.0 * kWindowWHOne];
    _inputPriceTextField2.keyboardType=UIKeyboardTypeDecimalPad;
    _inputPriceTextField2.textAlignment=NSTextAlignmentCenter;
    _inputPriceTextField2.enabled = NO;
    _inputPriceTextField2.hidden = YES;
    
    [backview1 addSubview:_inputPriceTextField2];
    
    [_inputPriceTextField2 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(5);
        make.top.mas_equalTo(7.5);
        make.height.mas_equalTo(20);
        make.right.equalTo(yline.mas_left).offset(-5);
    ...
    [_reduceBtn setImage:[UIImage imageNamed:NSLocalizedString(@"减", nil)] forState:UIControlStateNormal];
    ...
    _huilvlabel.text=NSLocalizedString(@"1张 = 100 USDT", nil);
    ...
    _znumberslabel.text=NSLocalizedString(@"0 张", nil);
...
-(void)initRightView{
    ...
    _rightnumberlabel.text= [NSString stringWithFormat:@"%@(BTC11)",LocalizationKey(@"1amount")]; //NSLocalizedString(@"数量(BTC)", nil);
    ...
    _leftpricelabel.text=[NSString stringWithFormat:@"%@(USDT)",LocalizationKey(@"tradePrice")];  //NSLocalizedString(@"价格(USDT)", nil);
...
///设置委托的状态，市价的时候不显示价格
-(void)setInputPriceTextField{
    if (_selectentrustIndex == 0) {//当前选择的是市价委托
        _inputPriceTextField.hidden = YES;
        _inputPriceTextField2.hidden = NO;
    }
    else{
        _inputPriceTextField.hidden = NO;
        _inputPriceTextField2.hidden = YES;
    }
}

-(void)setContractSoccket{
    NSLog(NSLocalizedString(@"U本位合约界面开始订阅socket", nil));
    ...
    [[SocketManager share] sendMsgWithLength:SOCKETREQUEST_LENGTH withsequenceId:0 withcmd:CONTRACT_SUBSCRIBE_SYMBOL_THUMB withVersion:COMMANDS_VERSION withRequestId:0 withbody:nil];
        ...
        if (!_isentrustType) {
            ...
            if (self.currentArray.count!=0&&indexPath.row<self.currentArray.count) {
                ...
                cell.pingBlock = ^{
                    ...
                    view.maxCount = model.resfreshTwo;
...
- (void)chexiaobtnclick:(UIButton *)button {
    ...
    ContractCurrtentEntrustTableViewCell *cell =(ContractCurrtentEntrustTableViewCell*) button.superview.superview;
...
-(void)loadBottomViewTableViewHeight{
    ...
    self.bottomBackView.frame=rect;
    self.myscollView.contentSize=CGSizeMake(SCREEN_WIDTH_S,rect.origin.y+rect.size.height + NEW_StatusBarHeight+MULTI_CONTRACT_TABBAR_HEIGHT);
}
            ...
            if ([resPonseObj[@"code"] intValue] == 0) {
                ...
                if ([resPonseObj isKindOfClass:[NSDictionary class]]) {
                    ...
                    if (![currentPrice isEqual:[NSNull null]]&&[currentPrice doubleValue]!=0) {
                        ...
                        _huilvlabel.text=[NSString stringWithFormat:@"1 %@ = %@ USDT",LocalizationKey(@"zhang"),self.symbolInfo[@"shareNumber"]];
...
-(NSInteger )getOpenBuyNumber {
    ...
    if ([self.coinInfo[@"usdtPattern"] isEqualToString:@"FIXED"]) {
        ...
        NSInteger number=[_leveragebuyStr integerValue]*[self.coinInfo[@"usdtBalance"] doubleValue]/[self.coinInfo[@"usdtShareNumber"] integerValue];
        return  number;
    }else{
        
        NSInteger number=0;
        double currencyprice= [self.inputPriceTextField.text doubleValue];
        double buypl=0;
        
        if ([self.coinInfo[@"usdtBuyPrice"] doubleValue]>0) {
            buypl= (currencyprice/[self.coinInfo[@"usdtBuyPrice"] doubleValue] -1)*([self.coinInfo[@"usdtBuyPosition"] doubleValue]+[self.coinInfo[@"usdtFrozenBuyPosition"] doubleValue]*[self.coinInfo[@"usdtShareNumber"] integerValue]);
        }
        double sellpl=0;
        if ([self.coinInfo[@"usdtSellPrice"] doubleValue]>0) {
            sellpl = (1- currencyprice/[self.coinInfo[@"usdtSellPrice"] doubleValue])*([self.coinInfo[@"usdtSellPosition"] doubleValue]+[self.coinInfo[@"usdtFrozenSellPosition"] doubleValue])*[self.coinInfo[@"usdtShareNumber"] integerValue];
        }
        ...
        if (lossandp>=0) {
            number=[self.coinInfo[@"usdtBuyLeverage"] integerValue]* [self.coinInfo[@"usdtBalance"] doubleValue]/[self.coinInfo[@"usdtShareNumber"] integerValue];
        }else
            number=[self.coinInfo[@"usdtBuyLeverage"] integerValue]* ([self.coinInfo[@"usdtBalance"] doubleValue]-lossandp)/[self.coinInfo[@"usdtShareNumber"] integerValue];
...
- (NSInteger) getOpenSellNumber {
    ...
    if ([self.coinInfo[@"usdtPattern"] isEqualToString:@"FIXED"]) {
        ...
        NSInteger number=[_leveragesellStr integerValue]*[self.coinInfo[@"usdtBalance"] doubleValue]/[self.coinInfo[@"usdtShareNumber"] integerValue];
        return  number;
    }else{
        ...
        if (lossandp>=0) {
            number=[self.coinInfo[@"usdtSellLeverage"] integerValue]* [self.coinInfo[@"usdtBalance"] doubleValue]/[self.coinInfo[@"usdtShareNumber"] integerValue];
        }else
            number=[self.coinInfo[@"usdtSellLeverage"] integerValue]* ([self.coinInfo[@"usdtBalance"] doubleValue]-lossandp)/[self.coinInfo[@"usdtShareNumber"] integerValue];
...
- (void)setNewModelPrice:(double)price {
    ...
    if ([self.coinInfo[@"usdtBuyPosition"] doubleValue]>0||[self.coinInfo[@"usdtFrozenBuyPosition"] doubleValue]>0) {
        ...
        double buyPL=(price/[self.coinInfo[@"usdtBuyPrice"] doubleValue]-1)*([self.coinInfo[@"usdtBuyPosition"]doubleValue]+[self.coinInfo[@"usdtFrozenBuyPosition"]doubleValue])*[self.coinInfo[@"usdtShareNumber"] integerValue];
    ...
    if ([self.coinInfo[@"usdtSellPosition"] doubleValue]>0||[self.coinInfo[@"usdtFrozenSellPosition"] doubleValue]>0){
        ...
        double sellpl=(1 - price/[self.coinInfo[@"usdtSellPrice"] doubleValue])*([self.coinInfo[@"usdtSellPosition"]doubleValue]+[self.coinInfo[@"usdtFrozenSellPosition"]doubleValue])*[self.coinInfo[@"usdtShareNumber"] integerValue];
        //收益
