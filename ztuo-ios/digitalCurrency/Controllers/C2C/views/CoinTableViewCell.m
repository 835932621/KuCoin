...
-(void)cellForArray:(NSArray *)payArr{
    if (payArr.count == 1) {
        if ([payArr[0] isEqualToString:LocalizationKey(@"支付宝")]) {
        ...
        }else if ([payArr[0] isEqualToString:LocalizationKey(@"微信")]){
        ...
        if ([payArr[0] isEqualToString:LocalizationKey(@"支付宝")]) {
            self.payWays1Width.constant = 20;
            if ([payArr[1] isEqualToString:LocalizationKey(@"微信")] ) {
                self.payWays2Width.constant = 20;
                self.payWay2Left.constant = 6;
                self.payWays3Width.constant = 0;
                self.payWay3Left.constant = 0;
            ...
            self.payWays2Width.constant = 20;
            self.payWay2Left.constant = 6;
            if ([payArr[1] isEqualToString:LocalizationKey(@"支付宝")]) {
                self.payWays1Width.constant = 20;
                self.payWays3Width.constant = 0;
                self.payWay3Left.constant = 0;
            }else{
