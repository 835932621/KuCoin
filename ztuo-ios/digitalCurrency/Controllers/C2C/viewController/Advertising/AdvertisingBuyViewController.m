...
//  [self RightsetupNavgationItemWithpictureName:@"helpImage"];
    ...
    if (_index == 1) {
        ...
        if (!_contentArr) {
            _contentArr = [NSMutableArray arrayWithArray:@[[[ChangeLanguage bundle] localizedStringForKey:@"select" value:nil table:@"English"],LocalizationKey(@"中国"),@"RMB",@"",@"MUTATIVE",@"",@"",@"",@"",@"",[[ChangeLanguage bundle] localizedStringForKey:@"select" value:nil table:@"English"],@"",@"",@"0",@"",@""]];
             [self editorInfo];
        }
    }
...
-(void)editorInfo{
    ...
    if (array.count > 0) {
        for (NSString *payStr in array) {
            if ([payStr isEqualToString:LocalizationKey(@"支付宝")]) {
                [payWaysArr addObject:LocalizationKey(@"alipay")];
            }else if ([payStr isEqualToString:LocalizationKey(@"微信")]){
...
-(void)selectCoinTypeData{
    ...
    [C2CNetManager selectCoinTypeForCompleteHandle:^(id resPonseObj, int code) {
        NSLog(LocalizationKey(@"选择币种的请求数据 --- %@"),resPonseObj);
