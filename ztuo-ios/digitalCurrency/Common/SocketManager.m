...
//  digitalCurrency
//
//  Created by sunliang on 2019/4/9.
//  Copyright © 2019年 BIZZAN. All rights reserved.
//

#import "SocketManager.h"
#import "SocketUtils.h"
//static  NSString * s_host = @"api.bizzan.pro";
//static  NSString * s_host = @"api.bc.com";
//static  NSString * s_host = @"api.yuylnt.cn";
...
- (BOOL)connect
{
    return  [gcdSocket connectToHost:SOCKET_HOST onPort:s_port error:nil];
}
...
//发送消息
- (void)sendMsgWithLength:(int)length withsequenceId:(long)sequenceId withcmd:(short)cmd withVersion:(int)Version withRequestId:(int)RequestId withbody:(NSDictionary* _Nullable)jsonDict{
...
- (void)socket:(GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port{
    NSLog(LocalizationKey(@"行情socket连接成功 port=%d"),port);
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err{
    NSLog(LocalizationKey(@"交易类socket断开连接%@"),err);
     //[APPLICATION.window makeToast:LocalizationKey(@"交易类socket断开连接") duration:5.5 position:CSToastPositionCenter];
...
//重连
-(void)reconnection{
     [gcdSocket connectToHost:SOCKET_HOST onPort:s_port error:nil];
}
