...
- (void)startCaptcha {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(delegateGtCaptcha: didReceiveCaptchaCode:result: message:)]){
        [_delegate delegateGtCaptcha:self.manager didReceiveCaptchaCode:@"1"  result:@{} message:@""];
    }
...
- (void)gtCaptcha:(GT3CaptchaManager *)manager errorHandler:(GT3Error *)error {
    //处理验证中返回的错误
    NSLog(LocalizationKey(@"验证出错：%ld"), error.code);
    if (error.code == -999) {
        // 请求被意外中断, 一般由用户进行取消操作导致, 可忽略错误
        [APPLICATION.window makeToast:LocalizationKey(@"请求被意外中断") duration:1.5 position:CSToastPositionCenter];
    }
    else if (error.code == -10) {
        // 预判断时被封禁, 不会再进行图形验证
        [APPLICATION.window makeToast:LocalizationKey(@"预判断时被封禁") duration:1.5 position:CSToastPositionCenter];
    }
    else if (error.code == -20) {
        // 尝试过多
        [APPLICATION.window makeToast:LocalizationKey(@"尝试过多") duration:1.5 position:CSToastPositionCenter];
    }
    else {
        // 网络问题或解析失败, 更多错误码参考开发文档
        [APPLICATION.window makeToast:LocalizationKey(@"网络问题或解析失败") duration:1.5 position:CSToastPositionCenter];
    }
    
}

- (void)gtCaptchaUserDidCloseGTView:(GT3CaptchaManager *)manager {
    NSLog(LocalizationKey(@"用户主动关闭了验证码界面"));
