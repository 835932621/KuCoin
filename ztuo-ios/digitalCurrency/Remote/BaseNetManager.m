...
//    NSLog(LocalizationKey(@"接口：%@ %@"),urlStr,para);
...
+(void)ylNonTokenRequestWithGET:(NSString *)urlString parameters:(NSDictionary *)parameter successBlock:(ResultBlock)resultBlock{
      NSMutableDictionary *para = [parameter mutableCopy];
    NSString *urlStr = urlStr = [NSString stringWithFormat:@"%@%@",HOST,urlString];
//    NSLog(LocalizationKey(@"接口&&参数%@%@"),urlStr,para);
...
//    NSLog(LocalizationKey(@"接口：%@ %@"),url,parameter);
    [task resume];
...
//        NSLog(@"~~~~~~~%@  %@", firstName, md5Str);
    ...
    [request addValue: langString forHTTPHeaderField:@"lang"];
    //设置body内容
    NSString *para = @"";
    for (int i =0; i<parameter.allKeys.count; i++) {
        NSString *key = parameter.allKeys[i];
...
//    NSURLSession *session = [NSURLSession sessionWithConfiguration:config];
    ...
    NSURLSession *session = [NSURLSession sessionWithConfiguration:config delegate:(id<NSURLSessionDelegate>)self delegateQueue:[[NSOperationQueue alloc]init]];
...
//        NSLog(LocalizationKey(@"darker 调用了里面这一层是服务器信任的证书"));
        /*
