...
//获取资产流水类型文本描述
+(void)reqAssettransTypeDesc:(void(^)(id resPonseObj,int code))completeHandle{
    NSString *path = [NSString stringWithFormat:@"%@%@",HOST,@"uc/asset/transaction/type"];

    [self requestWithPost:path header:nil parameters:@{} successBlock:^(id resultObject, int isSuccessed) {
        completeHandle(resultObject,isSuccessed);
    }];
...
//获取秒合约账户usdt
+(void)getMhyWalletDataCompleteHandle:(void(^)(id resPonseObj,int code))completeHandle{
    
    NSString *path = @"swap/wallet/second";
       NSMutableDictionary *dic = [NSMutableDictionary new];
       [self ylNonTokenRequestWithGET:path parameters:dic successBlock:^(id resultObject, int isSuccessed) {
           completeHandle(resultObject,isSuccessed);
       }];
...
///请求充值usdt
+(void)reqRecharge:(NSString*)count CompleteHandle:(void(^)(id resPonseObj,int code))completeHandle{
    NSString *path = @"uc/transfer/third";
    NSDictionary *dic = @{
                            @"amount":count
                        };
    [self ylNonTokenRequestWithGET:path parameters:dic successBlock:^(id resultObject, int isSuccessed) {
...
///请求提币
+(void)reqWithdraw:(NSInteger)count withAddress:(NSString*)walletAddress withCode:(NSString*)verifyCode withPassword:(NSString*)transferPassword CompleteHandle:(void(^)(id resPonseObj,int code))completeHandle{
    NSString *path = @"uc/withdraw/third";
    NSDictionary *dic = @{
                            @"amount":@(count),
...
///请求充值usdt记录
+(void)reqRechargeRecord:(void(^)(id resPonseObj,int code))completeHandle{
    NSString *path = @"uc/transfer/third/record";
    NSDictionary *dic = @{};
    [self ylNonTokenRequestWithGET:path parameters:dic successBlock:^(id resultObject, int isSuccessed) {
        completeHandle(resultObject, isSuccessed);
    }];
...
///请求提现usdt记录
+(void)reqWithdrawRecord:(void(^)(id resPonseObj,int code))completeHandle{
    NSString *path = @"uc/withdraw/third/record";
    NSDictionary *dic = @{};
    [self ylNonTokenRequestWithGET:path parameters:dic successBlock:^(id resultObject, int isSuccessed) {
        completeHandle(resultObject, isSuccessed);
    }];
...
///请求充币之前的数据
+(void)reqBeforeCharge:(void(^)(id resPonseObj,int code))completeHandle{
    NSString *path = @"uc/transfer/start";
    NSDictionary *dic = @{};
    [self ylNonTokenRequestWithGET:path parameters:dic successBlock:^(id resultObject, int isSuccessed) {
        completeHandle(resultObject, isSuccessed);
    }];
...
///上传充值订单截图
+(void)reqUploadChargeOrder:(NSString*)orderId imgUrl:(NSString*)transferImageUrl CompleteHandle:(void(^)(id resPonseObj,int code))completeHandle{
    NSString *path = @"uc/transfer/third/image/url";
    NSDictionary *dic = @{
                            @"orderId":orderId,
...
///客户端进行用户之间转账操作
+(void)reqTransferAssets:(NSString*)toUserName :(CGFloat)amount :(NSString*)jyPassword CompleteHandle:(void(^)(id resPonseObj,int code))completeHandle{
    NSString *path = @"uc/asset/wallet/transfer/member";
    NSDictionary *dic = @{
