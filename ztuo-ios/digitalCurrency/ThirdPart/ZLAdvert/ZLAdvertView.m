...
//        _adView.image = [UIImage imageNamed:@"LaunchImage_667h"];
        ...
        [_countBtn setTitle:[NSString stringWithFormat:LocalizationKey(@"跳过%d"), showtime] forState:UIControlStateNormal];
...
- (void)countDown {
    ...
    _count --;
    [_countBtn setTitle:[NSString stringWithFormat:LocalizationKey(@"跳过%d"),_count] forState:UIControlStateNormal];
    if (_count == 0) {
...
- (void)startCoundown {
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if(timeout <= 0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [_countBtn setTitle:[NSString stringWithFormat:LocalizationKey(@"跳过%d"),timeout] forState:UIControlStateNormal];
            });
            timeout--;
        }
    });
