...
- (NSNumber *)Volume_MA7
{
    if([Y_StockChartGlobalVariable isEMALine] == Y_StockChartTargetLineStatusMA)
    {
        if (!_Volume_MA7) {
            ...
            if (index >= 4) {
                if (index > 4) {
                    _Volume_MA7 = @((self.SumOfLastVolume.floatValue - self.ParentGroupModel.models[index - 5].SumOfLastVolume.floatValue) / 5);
                } else {
                    _Volume_MA7 = @(self.SumOfLastVolume.floatValue / 5);
                }
            }
        }
    } else {
...
{
    if(!_EMA30) {
        _EMA30 = @((2 * self.Close.floatValue + 9 * self.PreviousKlineModel.EMA30.floatValue)/11);
    }
    return _EMA30;
}
...
- (NSNumber *)MA30
{
    if([Y_StockChartGlobalVariable isEMALine] == Y_StockChartTargetLineStatusMA)
    {
        if (!_MA30) {
            ...
            if (index >= 9) {
                if (index > 9) {
                    _MA30 = @((self.SumOfLastClose.floatValue - self.ParentGroupModel.models[index - 10].SumOfLastClose.floatValue) / 10);
                } else {
                    _MA30 = @(self.SumOfLastClose.floatValue / 10);
                }
            }
        }
    } else {
...
- (NSNumber *)Volume_MA30
{
    if([Y_StockChartGlobalVariable isEMALine] == Y_StockChartTargetLineStatusMA)
    {
        if (!_Volume_MA30) {
            ...
            if (index >= 9) {
                if (index > 9) {
                    _Volume_MA30 = @((self.SumOfLastVolume.floatValue - self.ParentGroupModel.models[index - 10].SumOfLastVolume.floatValue) / 10);
                } else {
                    _Volume_MA30 = @(self.SumOfLastVolume.floatValue / 10);
                }
            }
        }
    } else {
...
{
    if(!_Volume_EMA30) {
        _Volume_EMA30 = @((2 * self.Volume + 9 * self.PreviousKlineModel.Volume_EMA30.floatValue)/11);
    }
...
- (void)rangeLastNinePriceByArray:(NSArray<Y_KLineModel *> *)models condition:(NSComparisonResult)cond
{
    ...
    switch (cond) {
        ...
        case NSOrderedAscending:
        {
            ...
            //第一个循环结束后，ClockFirstValue为最小值
            for (NSInteger i = 0, j = 6; j < models.count; i++,j++)
            {
                NSNumber *emMaxValue = @0;
...
{
    NSAssert(arr.count == 6, LocalizationKey(@"数组长度不足"));
