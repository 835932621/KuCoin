...
- (void)draw
{
    ...
    if (_MAType == Y_BOLL_DN || _MAType == Y_BOLL_MB || _MAType == Y_BOLL_UP){
        ...
        NSAssert(!isnan(firstPoint.x) && !isnan(firstPoint.y), LocalizationKey(@"出现NAN值：BOLL画线"));
        ...
        //NSAssert(!isnan(firstPoint.x) && !isnan(firstPoint.y), LocalizationKey(@"出现NAN值：MA画线"));
        CGContextMoveToPoint(self.context, firstPoint.x > 0 ? firstPoint.x : 0, firstPoint.y > 0 ? firstPoint.y : 0);
        
        for (NSInteger idx = 1; idx < self.MAPositions.count ; idx++)
        {
            CGPoint point = [self.MAPositions[idx] CGPointValue];
            CGContextAddLineToPoint(self.context, point.x > 0 ? point.x : 0, point.y > 0 ? point.y : 0);
            //
            //
            //        //日期
