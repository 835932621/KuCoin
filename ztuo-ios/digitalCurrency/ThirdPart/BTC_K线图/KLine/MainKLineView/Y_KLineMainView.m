...
- (void)drawRect:(CGRect)rect{
    ...
    CGContextFillRect(context, CGRectMake(0, Y_StockChartKLineMainViewMaxY + 1, self.frame.size.width, self.frame.size.height - Y_StockChartKLineMainViewMaxY - 1));
    
//    CGContextSetFillColorWithColor(context, kRGBColor(246, 246, 246).CGColor);
//    CGContextFillRect(context, CGRectMake(0, Y_StockChartKLineMainViewMaxY , self.frame.size.width, 1));
//    
//    CGContextSetFillColorWithColor(context, kRGBColor(246, 246, 246).CGColor);
...
- (void)drawMainView{
    NSAssert(self.kLineModels, LocalizationKey(@"kLineModels不能为空"));
...
- (void)updateMainViewWidth{
    ...
    //    if (offset > 0)
    //    {
    //        NSLog(LocalizationKey(@"计算出来的位移%f"),offset);
...
//    NSLog(LocalizationKey(@"这是模型开始的index-----------%lu"),needDrawKLineStartIndex);
...
- (NSArray *)private_convertToKLinePositionModelWithKLineModels {
    ...
    for (NSInteger idx = 0 ; idx < kLineModelsCount; ++idx)
    {
        ...
        NSAssert(!isnan(ma7Y) && !isnan(ma30Y), LocalizationKey(@"出现NAN值"));
...
//            NSLog(LocalizationKey(@"position：\n上: %@ \n中: %@ \n下: %@"),kLineModel.BOLL_UP,kLineModel.BOLL_MB,kLineModel.BOLL_DN);
            ...
            NSAssert(!isnan(boll_mbY) && !isnan(boll_upY) && !isnan(boll_dnY), LocalizationKey(@"出现BOLL值"));
