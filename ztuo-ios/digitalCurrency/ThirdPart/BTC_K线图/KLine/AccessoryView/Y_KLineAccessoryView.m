...
- (void)draw
{
    ...
    NSAssert(self.needDrawKLineModels && self.needDrawKLinePositionModels && self.kLineColors && kLineColorCount == kLineModelcount && kLinePositionModelCount == kLineModelcount, LocalizationKey(@"数据异常，无法绘制Volume"));
...
//                    DEAY = maxY - (model.DEA.floatValue - minValue)/unitValue;
            ...
            NSAssert(!isnan(DIFY) && !isnan(DEAY), LocalizationKey(@"出现NAN值"));
        ...
        [kLineModels enumerateObjectsUsingBlock:^(Y_KLineModel *  _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
           ...
           // NSAssert(!isnan(KDJ_K_Y) && !isnan(KDJ_D_Y) && !isnan(KDJ_J_Y), LocalizationKey(@"出现NAN值"));
