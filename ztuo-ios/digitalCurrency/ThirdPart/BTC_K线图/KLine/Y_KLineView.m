...
- (void)setTargetLineStatus:(Y_StockChartTargetLineStatus)targetLineStatus
{
    ...
    {    //关闭线
        if(targetLineStatus == Y_StockChartTargetLineStatusAccessoryClose){
            ...
            [Y_StockChartGlobalVariable setkLineVolumeViewRadio:0.31];
            self.accessoryMAView.hidden=YES;//副图
            self.accessoryView.hidden=YES; //副图的成交量
...
//        NSLog(LocalizationKey(@"原来的index%ld"),self.kLineMainView.needDrawStartIndex);
...
//            NSLog(LocalizationKey(@"计算得出的index%lu"),self.kLineMainView.pinchStartIndex);
        }
...
- (void)reDraw
{
    if (self.targetLineStatus==0) {
        self.targetLineStatus=102;//改成默认无副图
    }
...
{
    NSAssert(self.kLineVolumeView, LocalizationKey(@"kLineVolume不存在"));
    //更新约束
...
//    NSLog(LocalizationKey(@"这是  %f-----%f=====%f"),scrollView.contentSize.width,scrollView.contentOffset.x,self.kLineMainView.frame.size.width);
}
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView{
    
    NSNotification *notification =[NSNotification notificationWithName:@"HIDEINDEX" object:nil userInfo:nil];
