...
- (void)getDataSource
{
    _weekArray = @[LocalizationKey(@"日"), LocalizationKey(@"一"), LocalizationKey(@"二"), LocalizationKey(@"三"), LocalizationKey(@"四"), LocalizationKey(@"五"), LocalizationKey(@"六")];
...
- (void)creatControl
{
    ...
    [backTodayBtn setTitle:LocalizationKey(@"返回今天") forState:UIControlStateNormal];
    ...
    [sureBtn setTitle:LocalizationKey(@"确定") forState:UIControlStateNormal];
    ...
    [cancelBtn setTitle:LocalizationKey(@"取消") forState:UIControlStateNormal];
...
- (void)reloadData
{
    ...
    _monthLabel.text = [NSString stringWithFormat:LocalizationKey(@"%ld月"), _month];
    _yearBtn.title = [NSString stringWithFormat:LocalizationKey(@"%ld年"), _year];
    _monthBtn.title = [NSString stringWithFormat:LocalizationKey(@"%ld月"), _month];
        ...
        if (i < firstDay - 1 || i > totalDays + firstDay - 2) {
            ...
            if (_year == _currentYear && _month == _currentMonth) {
                if (btn.tag - KBtnTag - (firstDay - 2) == _currentDay) {
                    btn.selected = YES;
                    _day = _currentDay;
                    _weekLabel.text = [NSString stringWithFormat:LocalizationKey(@"星期%@"), _weekArray[(btn.tag - KBtnTag) % 7]];
