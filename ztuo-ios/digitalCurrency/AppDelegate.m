...
//    注册远程通知服务
    if (@available(iOS 10.0, *)) {
        ...
        [center requestAuthorizationWithOptions:(UNAuthorizationOptionBadge | UNAuthorizationOptionSound | UNAuthorizationOptionAlert) completionHandler:^(BOOL granted, NSError * _Nullable error) {
            if (!error && granted) {
                //用户点击允许
//                NSLog(LocalizationKey(@"注册成功"));
            }else{
                //用户点击不允许
                NSLog(LocalizationKey(@"注册失败"));
            }
        }];
...
-(void)customLaunchImageView{
    ...
    launchImageView.image = [UIImage imageNamed:@"启动页"]; //[self getLaunchImage];
...
//    NSString  *filePath = [[NSBundle bundleWithPath:[[NSBundle mainBundle] bundlePath]] pathForResource:@"启动页.png" ofType:nil];
...
//    if (isExist) { // 图片存在
//
       NSString  *filePath = [[NSBundle bundleWithPath:[[NSBundle mainBundle] bundlePath]] pathForResource:LocalizationKey(@"广告.png") ofType:nil];
...
//            NSLog(LocalizationKey(@"保存成功"));
//            [self deleteOldImage];
//            [kUserDefaults setValue:imageName forKey:adImageName];
//            [kUserDefaults synchronize];
//            // 如果有广告链接，将广告链接也保存下来
//        }else{
//            NSLog(LocalizationKey(@"保存失败"));
//        }
//
//    });
//}
...
//    NSLog(LocalizationKey(@"注册远程推送成功——————%@"),_deviceToken);
}
...
//        NSLog(LocalizationKey(@"远程推送的信息是--%@"),userInfo);
...
//        NSLog(LocalizationKey(@"订阅APNS"));

    }else if (cmd==UNSUBSCRIBE_APNS)
    {
//        NSLog(LocalizationKey(@"取消订阅APNS"));
    }
    else{
       // NSLog(LocalizationKey(@"聊天消息-%@--%d"),endStr,cmd);
     }
